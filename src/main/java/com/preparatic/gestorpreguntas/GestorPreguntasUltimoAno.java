...
public class GestorPreguntasUltimoAno extends GestorPreguntas {
	private static Logger logger = LogManager.getLogger(GestorPreguntasUltimoAno.class);
	 ...
	 */
	public GestorPreguntasUltimoAno(int anhoRevision) {
		super();
		CargarPreguntasAnho(anhoRevision);
		return;
	}

	private void CargarPreguntasAnho(int anhoRevision) {
		...
		try {
			...
			String sSql = "SELECT Count(*) AS TOTAL FROM `PREGUNTAS_TEST` where `REVISADA`='"
					+ anhoRevision + "';";
			ResultSet resultados = sentencia.executeQuery(sSql);
			resultados.next();
			if (resultados.getInt("TOTAL") == 0) {
				logger.info("No hay preguntas asociadas al test " + anhoRevision);
				resultados.close();
				sentencia.close();
				return;
			}
			...
			sSql = "SELECT `ID` FROM `PREGUNTAS_TEST` where `REVISADA`='"
					+ anhoRevision + "' ORDER BY `ANNO_CREACION` DESC;";
