...
//Se�ala en qu� pregunta se ha destapado la respuesta
...
function renderQuiz()
{
	for(var i=0;i<questions.length;i++)
	{
		...
		{
			document.writeln('<input type="radio" name="answer_' + i + '" value="' + choices[i][j] + '" id="answer_' + i + '_' + j + '" class="question_' + i + '" onclick="submitAnswer(' + i + ', this, \'question_' + i + '\', \'label_' + i + '_' + j + '\')" /><label id="label_' + i + '_' + j + '" for="answer_' + i + '_' + j + '"> ' + String.fromCharCode(letra) + ') ' + choices[i][j] + '</label><br />');
			letra = letra +1;
		}
		useranswers[i]=-1;
	}
...
function submitAnswer(questionId, obj, classId, labelId)
{
	...
	// Si est� mostrada la respuesta, entonces directamente lo corregimos
